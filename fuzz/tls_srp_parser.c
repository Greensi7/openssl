...
#define MAX_FUZZ_SRP_INPUT_LEN 4096

static void fuzz_srp(const uint8_t *buf, size_t len)
{
    SSL_CTX *ctx = NULL;
    SSL *ssl = NULL;
    ...
    tls_parse_ctos_srp(s, &pkt, 0, NULL, 0);
...
int FuzzerTestOneInput(const uint8_t *buf, size_t len)
{
    ...
    if (len == 0 || len > MAX_FUZZ_SRP_INPUT_LEN)
        return 0;
